...
import { getSonarrInstances } from "./config";
import { testConnection } from "./hooks/useSonarrAPI";
import type { SonarrInstance, HealthCheck } from "./types";
...
async function checkInstanceHealth(instance: SonarrInstance): Promise<SystemHealthSummary> {
  try {
...
function getOverallStatus(summaries: SystemHealthSummary[]): { style: Toast.Style; title: string } {
  ...
  if (hasWarnings) {
    return {
      ...
      title: "Sonarr Warnings",
    };
  }

  return {
    style: Toast.Style.Success,
    title: "All Sonarr Instances Healthy",
  };
}
...
export default async function SystemStatus() {
  try {
    ...
    if (instances.length === 0) {
      await showHUD("No Sonarr instances configured");
      return;
    }
    ...
    healthSummaries.forEach((summary) => {
      console.log(`Sonarr Instance: ${summary.instance.name}`);
